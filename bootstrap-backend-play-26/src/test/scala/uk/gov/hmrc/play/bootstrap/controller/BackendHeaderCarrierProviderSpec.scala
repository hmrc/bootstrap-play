...
class BackendHeaderCarrierProviderSpec extends WordSpec with TableDrivenPropertyChecks {
  ...
  classOf[BackendHeaderCarrierProvider].getSimpleName should {
    "create HeaderCarrier with path added to tags" in {
      val request       = FakeRequest(GET, "/the/request/path")
      val headerCarrier = backendHeaderCarrierProvider(request)
